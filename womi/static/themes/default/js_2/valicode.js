...
function sendMessage() {
  ...
  if($("input[id='signupform-email']").val()=="")
  {
    ...
    $.ajax({
    	type: "POST", //用POST方式传输
    	dataType: "text", 
    	//url: 'https://www.microriches.cn/index.php?r=site/valicode', //目标地址
    	url: 'http://www.microriches.cn/index.php?r=site/valicode', //目标地址
...
function checkCode(){
  ...
  if( valid && code != "")
  {
    ...
    $.ajax({
        type: "GET", 
        dataType: "text",
        //url: 'https://www.microriches.cn/index.php?r=site/valicode'+param, 
        url: 'http://www.microriches.cn/index.php?r=site/valicode'+param, 
        error: function (XMLHttpRequest, textStatus, errorThrown) {alert(XMLHttpRequest.status + ":" + XMLHttpRequest.statusText); },
        success: function (msg)
		 { 
			if(msg=='equal'){
