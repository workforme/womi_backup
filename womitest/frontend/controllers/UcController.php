...
class UcController extends BaseFrontController
{
	...
	public function actionShowdetail(){
		...
		$who=Yii::$app->user->identity->username;
                $model= User::findByUsername($who);
		
		if($model){
                	return $this->renderPartial($torend,['model' => $model]);		
		}else{
			return Yii::wmerr("找不到用户信息",'index.php?r=site/login',3);
		}
	...
	public function actionDetail(){
		...
		if(Yii::$app->user->isGuest){
			return Yii::success('请先登录','index.php?r=site/login',3);
		}
                $who=Yii::$app->user->identity->username;
		...
		if($model->load(Yii::$app->request->post()))
			...
			if($model->setDetail()){
			//失败	
				Yii::info($who." setDetail fail in  actionDetail");
				return "fail!";
			}else{
			//成功
				Yii::info($who." setDetail succ in  actionDetail");
				return "succ!";
			}
        	}
